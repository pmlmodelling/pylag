...
Note
----
waves_data_reader is implemented in Cython. Only a small portion of the
...
cdef class WavesDataReader(DataReader):
    """ WavesDataReader for wave model inputs on an Arakawa A-grid

    Objects of type WavesDataReader are intended to manage all access to
    wave data defined on a Arakawa A-grid, including data describing the
    ...
    def _read_grid(self):
        ...
        self._trim_first_latitude = self.mediator.get_grid_variable(
                'trim_first_latitude', (1), DTYPE_INT)[0]
        self._trim_last_latitude = self.mediator.get_grid_variable(
                'trim_last_latitude', (1), DTYPE_INT)[0]
        ...
        # Land sea mask
        try:
            use_mask = self.config.getboolean('WAVE_DATA', 'use_mask').strip()
    ...
    cdef _read_time_dependent_vars(self):
        ...
        vsdx_last = self.mediator.get_time_dependent_variable_at_last_time_index(
                vsdx_var_name, self._variable_shapes['vsdx'], DTYPE_FLOAT)
        self._vsdx_last = self._reshape_var(vsdx_last,
                self._variable_dimension_indices['vsdx'])
        del(vsdx_last)

        vsdx_next = self.mediator.get_time_dependent_variable_at_next_time_index(
                vsdx_var_name, self._variable_shapes['vsdx'], DTYPE_FLOAT)
