...
class MockArakawaAMediator(Mediator):
    ...
    def get_variable_dimensions(self, var_name, include_time=True):
        if include_time:
            ...
            # Time is the first dimension, so return all but the first dimension
...
class ArawawaADataReader_test(TestCase):
    ...
    def setUp(self):
        ...
        config.set('OCEAN_DATA', 'depth_dim_name', 'depth')
        config.set('OCEAN_DATA', 'latitude_dim_name', 'latitude')
        config.set('OCEAN_DATA', 'longitude_dim_name', 'longitude')
