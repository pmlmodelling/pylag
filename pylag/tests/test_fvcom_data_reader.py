...
from pylag.boundary_conditions import RefHorizCartesianBoundaryConditionCalculator as RefHorizBoundaryConditionCalculator
...
class MockFVCOMMediator(Mediator):
    ...
    def __init__(self):
        ...
        mask_c = np.array([0, 0, 2, 2, 2], dtype=int)
        mask = np.array([0, 0, 0, 0, 0, 0, 0], dtype=int)
        self._grid_vars = {'nv': nv, 'nbe': nbe, 'x': x, 'y': y, 'xc': xc,
                           'yc': yc, 'siglay': siglay, 'siglev': siglev,
                           'h': h, 'mask_c': mask_c, 'mask': mask}
        ...
        Kz_t0 = np.array([[0.]*n_nodes,[0.01]*n_nodes,[0.01]*n_nodes,[0.]*n_nodes], dtype=float)
        Kz_t1 = np.array([[0.]*n_nodes,[0.1]*n_nodes,[0.1]*n_nodes,[0.]*n_nodes], dtype=float)
        Kz = np.array([Kz_t0, Kz_t1], dtype=float)
